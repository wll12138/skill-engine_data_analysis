...
out = analyze_combustion_sensitivity(
    ...
    encoding="gbk",
    header_rows=5,
    save_plots=os.path.join(out_dir, "sensitivity/"),
)
print(out["report"])
...
- 燃烧基线诊断 — 调用 `single_engine_combustion_analysis()` 获得状态摘要
- Pearson + Spearman 相关 — 线性 + 单调非线性
- 互信息 — 信息论非线性 (需 scikit-learn)
- Random Forest 特征重要性 — impurity + permutation (需 scikit-learn)
- 偏依赖分析 — 特征→目标的非线性响应曲线 (需 scikit-learn)
- RPM / 负荷分区条件分析 — 参数影响随工况的反转点
- K-Fold 交叉验证 — 模型稳定性评估 (需 scikit-learn)
...
print(report)
with open(os.path.join(out_dir, "报告.md"), "w", encoding="utf-8") as f:
    f.write(report)
```
